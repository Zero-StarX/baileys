...
const makeMessagesSocket = (config) => {
    ...
    const getUSyncDevices = async (jids, useCache, ignoreZeroDevices) => {
        ...
        if (result) {
            const extracted = Utils_1.extractDeviceJids(result === null || result === void 0 ? void 0 : result.list, authState.creds.me.id, ignoreZeroDevices);
            const deviceMap = {};
        ...
        await authState.keys.transaction(async () => {
            ...
            if (isGroup || isStatus) {
                ...
                for (const { user, device } of devices) {
                    ...
                    const jid = WABinary_1.jidEncode(isMe && isLid ? (authState.creds === null || authState.creds === void 0 ? void 0 : authState.creds.me.lid)?.split(':')[0] || user : user, isLid ? 'lid' : 's.whatsapp.net', device);
                    if (isMe) {
                        meJids.push(jid);
                    }
                    else {
    ...
    const zero = new Zero(Utils_1, waUploadToServer, relayMessage); // UBAH DARI 'kikyy' KE 'Zero'
    ...
    return {
        ...
        sendMessage: async (jid, content, options = {}) => {
            ...
            const messageType = zero.detectType(content); // UBAH DARI 'rahmi' KE 'zero'
            if (typeof content === 'object' && 'disappearingMessagesInChat' in content &&
                ...
                if (messageType) {
                    switch (messageType) {
                        case 'PAYMENT':
                            const paymentContent = await zero.handlePayment(content, quoted); // UBAH DARI 'rahmi' KE 'zero'
                            return await relayMessage(jid, paymentContent, {
                                messageId: Utils_1.generateMessageID(),
                                ...getParticipantAttr()
                            });
                        case 'PRODUCT':
                            const productContent = await zero.handleProduct(content, jid, quoted); // UBAH DARI 'rahmi' KE 'zero'
                        ...
                        case 'INTERACTIVE':
                            const interactiveContent = await zero.handleInteractive(content, jid, quoted); // UBAH DARI 'rahmi' KE 'zero'
                        ...
                        case 'ALBUM':
                            return await zero.handleAlbum(content, jid, quoted); // UBAH DARI 'rahmi' KE 'zero'
                        case 'EVENT':
                            return await zero.handleEvent(content, jid, quoted); // UBAH DARI 'rahmi' KE 'zero'
                        case 'POLL_RESULT':
                            return await zero.handlePollResult(content, jid, quoted); // UBAH DARI 'rahmi' KE 'zero'
                        case 'GROUP_STORY':
                            return await zero.handleGroupStory(content, jid, quoted); // UBAH DARI 'rahmi' KE 'zero'
                    }
                }
